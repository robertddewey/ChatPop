...
This document outlines the comprehensive test plan for the `media_analysis` Django app, which provides AI-powered photo analysis using OpenAI GPT-4o Vision API to generate contextual chat room name suggestions.
...
- `test_media_analysis_creation` - Model creates successfully
- `test_media_analysis_uuid_primary_key` - UUID auto-generated
- `test_media_analysis_is_expired_returns_true_when_expired` - Expiration check works
- `test_media_analysis_is_expired_returns_false_when_active` - Active images not expired
- `test_media_analysis_is_expired_returns_false_when_no_expiry` - Null expiry never expires
- `test_media_analysis_increment_usage` - Usage counter increments
- `test_media_analysis_stores_suggestions_as_json` - JSON field works
- `test_media_analysis_stores_user_tracking` - User/fingerprint/IP stored
...
- `test_media_analysis_detail_serializer_includes_image_url` - URL generated
- `test_media_analysis_list_serializer_includes_count` - Suggestion count included
...
```bash
# Skip by default
./venv/bin/python manage.py test media_analysis  # integration tests NOT run

# Run ONLY integration tests
pytest -m integration media_analysis/tests/test_vision_api_integration.py

# Run ALL tests including integration
pytest media_analysis
```
...
def mock_redis(mocker):
    ...
    return mocker.patch('media_analysis.utils.rate_limit.redis_client')
...
```bash
cd backend
./venv/bin/python manage.py test media_analysis
# Integration tests are automatically skipped
```

### Run Specific Test Module
```bash
./venv/bin/python manage.py test media_analysis.tests.test_fingerprinting
```
...
pytest -m integration media_analysis/tests/test_vision_api_integration.py::test_integration_real_api_analyzes_image_successfully
```
...
```bash
coverage run --source='media_analysis' manage.py test media_analysis
coverage report
...
```bash
./venv/bin/python manage.py test media_analysis -v 2
```
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run media_analysis tests
        run: |
          cd backend
          python manage.py test media_analysis
```
