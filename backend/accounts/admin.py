...
class UserAdmin(BaseUserAdmin):
    list_display = ['email', 'reserved_username', 'is_staff', 'subscriber_count', 'subscription_count', 'created_at']
    list_filter = ['is_staff', 'is_active', 'created_at']
    search_fields = ['email', 'reserved_username']
    ordering = ['-created_at']

    fieldsets = (
        (None, {'fields': ('email', 'password')}),
        ('Personal Info', {'fields': ('reserved_username', 'first_name', 'last_name')}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'password1', 'password2', 'reserved_username'),
        }),
    )
