...
export default function CreateChatModal({ onClose }: CreateChatModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75">
      ...
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl p-8 relative max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-lg transition-colors text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800"
          aria-label="Close"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-zinc-700 rounded-xl focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-colors focus:outline-none bg-zinc-800 text-zinc-100 placeholder-zinc-500"
              placeholder=""
            />
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <div className="font-semibold text-zinc-100">Public</div>
              ...
              >
                <div className="font-semibold text-zinc-100">Private</div>
                <div className="text-sm text-zinc-400">Requires access code</div>
              </button>
            </div>
          </div>
          ...
          {formData.access_mode === 'private' && (
            <div>
              <label htmlFor="access_code" className="block text-sm font-bold text-zinc-300 mb-2">
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 text-cyan-400 border-zinc-700 rounded focus:ring-cyan-400"
                />
                <span className="text-zinc-300">Photo Sharing</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 text-cyan-400 border-zinc-700 rounded focus:ring-cyan-400"
                />
                <span className="text-zinc-300">Voice Messages</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 text-cyan-400 border-zinc-700 rounded focus:ring-cyan-400"
                />
                <span className="text-zinc-300">Video Messages</span>
              </label>
            </div>
          </div>
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={loading}
              className="w-full px-6 py-3 bg-[#404eed] text-white font-semibold rounded-lg hover:bg-[#3640d9] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
