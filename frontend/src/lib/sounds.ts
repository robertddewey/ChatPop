/**
 * Sound utility functions for ChatPop
 * Uses HTML5 Audio elements with dynamically generated WAV files
 * iOS Safari compatible - audio unlocked during "Join Chat" user gesture
 *
 * iOS requires audio element pooling to play the same sound consecutively.
 * We pre-create multiple Audio elements during the user gesture and rotate through them.
 */

// Singleton AudioContext that persists across the app (used for fallback)
...
// Also pre-creates audio element pools for iOS compatibility
export const initAudioContext = async (): Promise<AudioContext | null> => {
  try {
    ...
    // Pre-create audio element pools during user gesture for iOS compatibility
    // This allows consecutive plays of the same sound by rotating through elements
    if (receiveAudioPool.length === 0) {
      const receiveSound = generateReceiveMessageSound();
      for (let i = 0; i < POOL_SIZE; i++) {
        const audio = new Audio(receiveSound);
        audio.volume = 0.6;
        receiveAudioPool.push(audio);
      }
    }

    if (sendAudioPool.length === 0) {
      const sendSound = generateSendMessageSound();
      for (let i = 0; i < POOL_SIZE; i++) {
        const audio = new Audio(sendSound);
        audio.volume = 0.6;
        sendAudioPool.push(audio);
      }
    }
...
// Generate send message sound (ascending C major arpeggio - same as join sound)
const generateSendMessageSound = (): string => {
  const sampleRate = 44100;
  const duration = 0.4; // 400ms
  const numSamples = Math.floor(sampleRate * duration);
...
// Generate receive message sound (descending C major arpeggio - inverse of send)
const generateReceiveMessageSound = (): string => {
  const sampleRate = 44100;
  const duration = 0.4; // 400ms
  const numSamples = Math.floor(sampleRate * duration);
  ...
  const samples = new Int16Array(numSamples);
  const notes = [783.99, 659.25, 523.25]; // G5, E5, C5 (descending - inverse)
  const noteDuration = duration / 4;
...
// Play sound when sending a message (ascending arpeggio)
// Uses audio pool for iOS compatibility
export const playSendMessageSound = async () => {
  try {
    // Use pooled audio element if available (iOS compatibility)
    if (sendAudioPool.length > 0) {
      const audio = sendAudioPool[sendPoolIndex];
      audio.currentTime = 0; // Reset to start
      await audio.play();
      sendPoolIndex = (sendPoolIndex + 1) % sendAudioPool.length;
    } else {
      // Fallback: create new Audio element (works on desktop, may fail on iOS)
      const audio = new Audio();
      audio.src = generateSendMessageSound();
      audio.volume = 0.6;
      await audio.play();
    }
  } catch (error) {
    console.log('Send message sound failed:', error);
  }
};

// Play sound when receiving a message (descending arpeggio)
// Uses audio pool for iOS compatibility - rotates through pre-created elements
export const playReceiveMessageSound = async () => {
  try {
    // Use pooled audio element if available (iOS compatibility)
    if (receiveAudioPool.length > 0) {
      const audio = receiveAudioPool[receivePoolIndex];
      audio.currentTime = 0; // Reset to start
      await audio.play();
      receivePoolIndex = (receivePoolIndex + 1) % receiveAudioPool.length;
    } else {
      // Fallback: create new Audio element (works on desktop, may fail on iOS)
      const audio = new Audio();
      audio.src = generateReceiveMessageSound();
      audio.volume = 0.6;
      await audio.play();
    }
  } catch (error) {
    console.log('Receive message sound failed:', error);
  }
};
