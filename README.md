...
| `docker` | Local Redis (required for the app). Postgres only on demand for `manage.py test` or `chatpop seed from-local`. |
...
# Step 6: Install dev-robert keys into your chatpop-dev profile
chatpop admin install-dev-keys robert

# Step 7: Tailscale signin, identity file, cloud .env config, git hooks
chatpop join
...
├── docs/                # Documentation
├── docker-compose.yml   # Redis (default). Postgres in 'local-tests' profile (opt-in)
